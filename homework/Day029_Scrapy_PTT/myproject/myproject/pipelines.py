...
class JSONPipeline(object):
    ...
    def close_spider(self, spider):
        ...
        if spider.name == 'PTTCrawler' and spider.board:
            if Path(spider.board).suffix == '.json':     # 後綴.json直接加在目錄
                self.store_file_path = self.dir_path / spider.board
            else:
                self.store_file_path = self.dir_path / '{}.json'.format(spider.board)
